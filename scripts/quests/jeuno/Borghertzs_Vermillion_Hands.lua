...
local quest = Quest:new(xi.questLog.JEUNO, xi.quest.id.jeuno.BORGHERTZS_VERMILLION_HANDS)
...
local function canStartQuest(player)
    ...
    if player:getQuestStatus(xi.questLog.SANDORIA, xi.quest.id.sandoria.ENVELOPED_IN_DARKNESS) == xi.questStatus.QUEST_AVAILABLE then
        return false
    end
...
quest.sections =
{
...

    {
        ...
        [xi.zone.CASTLE_OZTROJA] =
        {
            ...
            {
                onTrade = function(player, npc, trade)
                    if not player:hasItem(xi.item.WARLOCKS_TABARD) then
                        xi.treasure.onTrade(player, npc, trade, 1, xi.item.WARLOCKS_TABARD)
        ...
        [xi.zone.GARLAIGE_CITADEL] =
        {
            ...
            {
                onTrade = function(player, npc, trade)
                    if not player:hasItem(xi.item.WARLOCKS_TIGHTS) then
                        xi.treasure.onTrade(player, npc, trade, 1, xi.item.WARLOCKS_TIGHTS)
