...
local quest = Quest:new(xi.questLog.JEUNO, xi.quest.id.jeuno.BORGHERTZS_STRIKING_HANDS)
...
local function canStartQuest(player)
    ...
    if player:getQuestStatus(xi.questLog.BASTOK, xi.quest.id.bastok.THE_FIRST_MEETING) == xi.questStatus.QUEST_AVAILABLE then
        return false
    end
...
quest.sections =
{
...

    {
        ...
        [xi.zone.BEADEAUX] =
        {
            ...
            {
                onTrade = function(player, npc, trade)
                    if not player:hasItem(xi.item.TEMPLE_CYCLAS) then
                        xi.treasure.onTrade(player, npc, trade, 1, xi.item.TEMPLE_CYCLAS)
        ...
        [xi.zone.GARLAIGE_CITADEL] =
        {
            ...
            {
                onTrade = function(player, npc, trade)
                    if not player:hasItem(xi.item.TEMPLE_CROWN) then
                        xi.treasure.onTrade(player, npc, trade, 1, xi.item.TEMPLE_CROWN)
