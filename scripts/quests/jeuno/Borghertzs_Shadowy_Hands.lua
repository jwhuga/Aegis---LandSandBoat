...
local quest = Quest:new(xi.questLog.JEUNO, xi.quest.id.jeuno.BORGHERTZS_SHADOWY_HANDS)
...
local function canStartQuest(player)
    ...
    if player:getQuestStatus(xi.questLog.BASTOK, xi.quest.id.bastok.DARK_PUPPET) == xi.questStatus.QUEST_AVAILABLE then
        return false
    end
...
quest.sections =
{
...

    {
        ...
        [xi.zone.CASTLE_OZTROJA] =
        {
            ...
            {
                onTrade = function(player, npc, trade)
                    if not player:hasItem(xi.item.CHAOS_CUIRASS) then
                        xi.treasure.onTrade(player, npc, trade, 1, xi.item.CHAOS_CUIRASS)
        ...
        [xi.zone.MONASTIC_CAVERN] =
        {
            ...
            {
                onTrade = function(player, npc, trade)
                    if not player:hasItem(xi.item.CHAOS_FLANCHARD) then
                        xi.treasure.onTrade(player, npc, trade, 1, xi.item.CHAOS_FLANCHARD)
