...
local quest = Quest:new(xi.questLog.JEUNO, xi.quest.id.jeuno.BORGHERTZS_STALWART_HANDS)
...
local function canStartQuest(player)
    ...
    if player:getQuestStatus(xi.questLog.SANDORIA, xi.quest.id.windurst.A_BOYS_DREAM) == xi.questStatus.QUEST_AVAILABLE then
        return false
    end
...
quest.reward =
{
    item = xi.item.GALLANT_GAUNTLETS,
}
...
quest.sections =
{
...

    {
        ...
        [xi.zone.BEADEAUX] =
        {
            ...
            {
                onTrade = function(player, npc, trade)
                    if not player:hasItem(xi.item.GALLANT_BREECHES) then
                        xi.treasure.onTrade(player, npc, trade, 1, xi.item.GALLANT_BREECHES)
        ...
        [xi.zone.GARLAIGE_CITADEL] =
        {
            ...
            {
                onTrade = function(player, npc, trade)
                    if not player:hasItem(xi.item.GALLANT_CORONET) then
                        xi.treasure.onTrade(player, npc, trade, 1, xi.item.GALLANT_CORONET)
