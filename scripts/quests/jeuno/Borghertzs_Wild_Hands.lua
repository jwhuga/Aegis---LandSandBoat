...
local quest = Quest:new(xi.questLog.JEUNO, xi.quest.id.jeuno.BORGHERTZS_WILD_HANDS)
...
local function canStartQuest(player)
    ...
    if player:getQuestStatus(xi.questLog.JEUNO, xi.quest.id.jeuno.SCATTERED_INTO_SHADOW) == xi.questStatus.QUEST_AVAILABLE then
        return false
    end
...
quest.sections =
{
...

    {
        ...
        [xi.zone.BEADEAUX] =
        {
            ...
            {
                onTrade = function(player, npc, trade)
                    if not player:hasItem(xi.item.BEAST_JACKCOAT) then
                        xi.treasure.onTrade(player, npc, trade, 1, xi.item.BEAST_JACKCOAT)
        ...
        [xi.zone.GARLAIGE_CITADEL] =
        {
            ...
            {
                onTrade = function(player, npc, trade)
                    if not player:hasItem(xi.item.BEAST_HELM) then
                        xi.treasure.onTrade(player, npc, trade, 1, xi.item.BEAST_HELM)
