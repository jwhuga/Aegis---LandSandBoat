...
local quest = Quest:new(xi.questLog.JEUNO, xi.quest.id.jeuno.BORGHERTZS_SORCEROUS_HANDS)
...
local function canStartQuest(player)
    ...
    if player:getQuestStatus(xi.questLog.WINDURST, xi.quest.id.windurst.RECOLLECTIONS) == xi.questStatus.QUEST_AVAILABLE then
        return false
    end
...
quest.sections =
{
...

    {
        ...
        [xi.zone.MONASTIC_CAVERN] =
        {
            ...
            {
                onTrade = function(player, npc, trade)
                    if not player:hasItem(xi.item.WIZARDS_COAT) then
                        xi.treasure.onTrade(player, npc, trade, 1, xi.item.WIZARDS_COAT)
        ...
        [xi.zone.THE_ELDIEME_NECROPOLIS] =
        {
            ...
            {
                onTrade = function(player, npc, trade)
                    if not player:hasItem(xi.item.WIZARDS_TONBAN) then
                        xi.treasure.onTrade(player, npc, trade, 1, xi.item.WIZARDS_TONBAN)
