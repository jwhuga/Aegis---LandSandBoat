...
local quest = Quest:new(xi.questLog.JEUNO, xi.quest.id.jeuno.BORGHERTZS_HEALING_HANDS)
...
local function canStartQuest(player)
    ...
    if player:getQuestStatus(xi.questLog.SANDORIA, xi.quest.id.sandoria.PRELUDE_OF_BLACK_AND_WHITE) == xi.questStatus.QUEST_AVAILABLE then
        return false
    end
...
quest.sections =
{
...

    {
        ...
        [xi.zone.CRAWLERS_NEST] =
        {
            ...
            {
                onTrade = function(player, npc, trade)
                    if not player:hasItem(xi.item.HEALERS_PANTALOONS) then
                        xi.treasure.onTrade(player, npc, trade, 1, xi.item.HEALERS_PANTALOONS)
        ...
        [xi.zone.GARLAIGE_CITADEL] =
        {
            ...
            {
                onTrade = function(player, npc, trade)
                    if not player:hasItem(xi.item.HEALERS_CAP) then
                        xi.treasure.onTrade(player, npc, trade, 1, xi.item.HEALERS_CAP)
