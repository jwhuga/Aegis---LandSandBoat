...
local quest = Quest:new(xi.questLog.JEUNO, xi.quest.id.jeuno.BORGHERTZS_CALLING_HANDS)
...
local function canStartQuest(player)
    ...
    if player:getQuestStatus(xi.questLog.WINDURST, xi.quest.id.windurst.CLASS_REUNION) == xi.questStatus.QUEST_AVAILABLE then
        return false
    end
...
quest.sections =
{
...

    {
        ...
        [xi.zone.TEMPLE_OF_UGGALEPIH] =
        {
            ...
            {
                onTrade = function(player, npc, trade)
                    if not player:hasItem(xi.item.EVOKERS_DOUBLET) then
                        xi.treasure.onTrade(player, npc, trade, 1, xi.item.EVOKERS_DOUBLET)
        ...
        [xi.zone.TORAIMARAI_CANAL] =
        {
            ...
            {
                onTrade = function(player, npc, trade)
                    if not player:hasItem(xi.item.EVOKERS_PIGACHES) then
                        xi.treasure.onTrade(player, npc, trade, 1, xi.item.EVOKERS_PIGACHES)
