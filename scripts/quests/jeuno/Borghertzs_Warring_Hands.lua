...
local quest = Quest:new(xi.questLog.JEUNO, xi.quest.id.jeuno.BORGHERTZS_WARRING_HANDS)
...
local function canStartQuest(player)
    ...
    if player:getQuestStatus(xi.questLog.BASTOK, xi.quest.id.bastok.THE_TALEKEEPERS_TRUTH) == xi.questStatus.QUEST_AVAILABLE then
        return false
    end
...
quest.reward =
{
    item = xi.item.FIGHTERS_MUFFLERS,
}
...
quest.sections =
{
...

    {
        ...
        [xi.zone.CASTLE_ZVAHL_BAILEYS] =
        {
            ...
            {
                onTrade = function(player, npc, trade)
                    if not player:hasItem(xi.item.FIGHTERS_CUISSES) then
                        xi.treasure.onTrade(player, npc, trade, 1, xi.item.FIGHTERS_CUISSES)
        ...
        [xi.zone.CRAWLERS_NEST] =
        {
            ...
            {
                onTrade = function(player, npc, trade)
                    if not player:hasItem(xi.item.FIGHTERS_MASK) then
                        xi.treasure.onTrade(player, npc, trade, 1, xi.item.FIGHTERS_MASK)
