...
local quest = Quest:new(xi.questLog.JEUNO, xi.quest.id.jeuno.BORGHERTZS_LURKING_HANDS)
...
local function canStartQuest(player)
    ...
    if player:getQuestStatus(xi.questLog.OUTLANDS, xi.quest.id.outlands.I_LL_TAKE_THE_BIG_BOX) == xi.questStatus.QUEST_AVAILABLE then
        return false
    end
...
quest.sections =
{
...

    {
        ...
        [xi.zone.SEA_SERPENT_GROTTO] =
        {
            ...
            {
                onTrade = function(player, npc, trade)
                    if not player:hasItem(xi.item.NINJA_KYAHAN) then
                        xi.treasure.onTrade(player, npc, trade, 1, xi.item.NINJA_KYAHAN)
        ...
        [xi.zone.THE_BOYAHDA_TREE] =
        {
            ...
            {
                onTrade = function(player, npc, trade)
                    if not player:hasItem(xi.item.NINJA_HATSUBURI) then
                        xi.treasure.onTrade(player, npc, trade, 1, xi.item.NINJA_HATSUBURI)
