...
local quest = Quest:new(xi.questLog.JEUNO, xi.quest.id.jeuno.BORGHERTZS_SNEAKY_HANDS)
...
local function canStartQuest(player)
    ...
    if player:getQuestStatus(xi.questLog.WINDURST, xi.quest.id.windurst.AS_THICK_AS_THIEVES) == xi.questStatus.QUEST_AVAILABLE then
        return false
    end
...
quest.sections =
{
...

    {
        ...
        [xi.zone.CASTLE_OZTROJA] =
        {
            ...
            {
                onTrade = function(player, npc, trade)
                    if not player:hasItem(xi.item.ROGUES_CULOTTES) then
                        xi.treasure.onTrade(player, npc, trade, 1, xi.item.ROGUES_CULOTTES)
        ...
        [xi.zone.CASTLE_ZVAHL_BAILEYS] =
        {
            ...
            {
                onTrade = function(player, npc, trade)
                    if not player:hasItem(xi.item.ROGUES_VEST) then
                        xi.treasure.onTrade(player, npc, trade, 1, xi.item.ROGUES_VEST)
