...
local quest = Quest:new(xi.questLog.JEUNO, xi.quest.id.jeuno.BORGHERTZS_DRAGON_HANDS)
...
local function canStartQuest(player)
    ...
    if player:getQuestStatus(xi.questLog.SANDORIA, xi.quest.id.sandoria.CHASING_QUOTAS) == xi.questStatus.QUEST_AVAILABLE then
        return false
    end
...
quest.reward =
{
    item = xi.item.DRACHEN_FINGER_GAUNTLETS,
}
...
quest.sections =
{
...

    {
        ...
        [xi.zone.IFRITS_CAULDRON] =
        {
            ...
            {
                onTrade = function(player, npc, trade)
                    if not player:hasItem(xi.item.DRACHEN_MAIL) then
                        xi.treasure.onTrade(player, npc, trade, 1, xi.item.DRACHEN_MAIL)
        ...
        [xi.zone.QUICKSAND_CAVES] =
        {
            ...
            {
                onTrade = function(player, npc, trade)
                    if not player:hasItem(xi.item.DRACHEN_GREAVES) then
                        xi.treasure.onTrade(player, npc, trade, 1, xi.item.DRACHEN_GREAVES)
