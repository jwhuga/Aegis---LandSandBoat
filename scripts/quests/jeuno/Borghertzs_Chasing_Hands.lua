...
local quest = Quest:new(xi.questLog.JEUNO, xi.quest.id.jeuno.BORGHERTZS_CHASING_HANDS)
...
quest.sections =
{
...

    {
        ...
        [xi.zone.CRAWLERS_NEST] =
        {
            ...
            {
                onTrade = function(player, npc, trade)
                    if not player:hasItem(xi.item.HUNTERS_BRACCAE) then
                        xi.treasure.onTrade(player, npc, trade, 1, xi.item.HUNTERS_BRACCAE)
        ...
        [xi.zone.MONASTIC_CAVERN] =
        {
            ...
            {
                onTrade = function(player, npc, trade)
                    if not player:hasItem(xi.item.HUNTERS_JERKIN) then
                        xi.treasure.onTrade(player, npc, trade, 1, xi.item.HUNTERS_JERKIN)
