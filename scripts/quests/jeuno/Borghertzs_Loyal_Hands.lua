...
local quest = Quest:new(xi.questLog.JEUNO, xi.quest.id.jeuno.BORGHERTZS_LOYAL_HANDS)
...
local function canStartQuest(player)
    ...
    if player:getQuestStatus(xi.questLog.OUTLANDS, xi.quest.id.outlands.YOMI_OKURI) == xi.questStatus.QUEST_AVAILABLE then
        return false
    end
...
quest.sections =
{
...

    {
        ...
        [xi.zone.QUICKSAND_CAVES] =
        {
            ...
            {
                onTrade = function(player, npc, trade)
                    if not player:hasItem(xi.item.MYOCHIN_HAIDATE) then
                        xi.treasure.onTrade(player, npc, trade, 1, xi.item.MYOCHIN_HAIDATE)
        ...
        [xi.zone.TEMPLE_OF_UGGALEPIH] =
        {
            ...
            {
                onTrade = function(player, npc, trade)
                    if not player:hasItem(xi.item.MYOCHIN_DOMARU) then
                        xi.treasure.onTrade(player, npc, trade, 1, xi.item.MYOCHIN_DOMARU)
